...
uint32_t G711ASource::GetSampleRate() const
{
	return samplerate_;
}

uint32_t G711ASource::GetChannels() const
{
	return channels_;
}
...
bool G711ASource::HandleFrame(MediaChannelId channel_id, AVFrame& frame)
{
	...
	std::memcpy(rtp_pkt.data.get()+4+RTP_HEADER_SIZE, frame_buf, frame_size);
...
constexpr uint32_t G711ASource::GetTimestamp()
{
