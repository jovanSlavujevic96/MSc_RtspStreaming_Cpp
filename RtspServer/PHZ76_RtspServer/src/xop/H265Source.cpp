...
void H265Source::Setframerate(uint32_t framerate)
{
	framerate_ = framerate;
}

uint32_t H265Source::GetFramerate() const
{
	return framerate_;
}
...
bool H265Source::HandleFrame(MediaChannelId channelId, AVFrame& frame)
{
	...
	if (frame_size <= MAX_RTP_PAYLOAD_SIZE) {
...

		{
			...
			rtp_pkt.data.get()[RTP_HEADER_SIZE+5] = FU[1];
			rtp_pkt.data.get()[RTP_HEADER_SIZE+6] = FU[2];
			std::memcpy(rtp_pkt.data.get()+4+RTP_HEADER_SIZE+3, frame_buf, frame_size);
...
constexpr uint32_t H265Source::GetTimestamp()
{
/* #if defined(__linux) || defined(__linux__) 
	struct timeval tv = {0};
