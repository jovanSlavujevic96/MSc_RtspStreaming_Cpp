...
std::random_device RtpConnection::rd;
#endif

RtpConnection::RtpConnection(std::weak_ptr<net::TcpConnection> rtsp_connection)
...
RtpConnection::~RtpConnection()
{
	for(int chn=0; chn<MAX_MEDIA_CHANNEL; chn++) 
	{
		if(rtpfd_[chn] > 0) 
		{
			net::SocketUtil::Close(rtpfd_[chn]);
		}

		if(rtcpfd_[chn] > 0) 
		{
			net::SocketUtil::Close(rtcpfd_[chn]);
		}
	}
}
...
int RtpConnection::GetId() const
{
	...
	return ((RtspConnection*)conn.get())->GetId();
}
...
bool RtpConnection::SetupRtpOverUdp(MediaChannelId channel_id, uint16_t rtp_port, uint16_t rtcp_port)
{
	auto conn = rtsp_connection_.lock();
	if ( (!conn) || 
		 (net::SocketUtil::GetPeerAddr(conn->GetSocket(), &peer_addr_) < 0) )
	{
		return false;
	}
	...
	for (int n = 0; n <= 10; n++) 
	{
		...
		if(!net::SocketUtil::Bind(rtpfd_[channel_id], "0.0.0.0",  local_rtp_port_[channel_id])) 
		{
			net::SocketUtil::Close(rtpfd_[channel_id]);
			continue;
		}

		rtcpfd_[channel_id] = ::socket(AF_INET, SOCK_DGRAM, 0);
		if(!net::SocketUtil::Bind(rtcpfd_[channel_id], "0.0.0.0", local_rtcp_port_[channel_id]))
		{
			net::SocketUtil::Close(rtpfd_[channel_id]);
			net::SocketUtil::Close(rtcpfd_[channel_id]);
			continue;
		}
	...
	net::SocketUtil::SetSendBufSize(rtpfd_[channel_id], 50*1024);
...
std::string RtpConnection::GetRtpInfo(const std::string& rtsp_url)
{
	...
	const auto time_point = std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now());
...
void RtpConnection::SetFrameType(uint8_t frame_type)
{
	...
	if(!has_key_frame_ && (frame_type == 0 || frame_type == (uint8_t)FrameType::VIDEO_FRAME_I)) 
	{
		has_key_frame_ = true;
	}
}
...
{    
	static RtspConnection* rtsp_conn;
	if (is_closed_) 
	{
		return -1;
	}

	SetFrameType(pkt.type);
	SetRtpHeader(channel_id, pkt);

	if ((media_channel_info_[channel_id].is_play || media_channel_info_[channel_id].is_record) && has_key_frame_)
	{
		return (transport_mode_ == RTP_OVER_TCP) ? SendRtpOverTcp(channel_id, pkt) : SendRtpOverUdp(channel_id, pkt);
	}
	return -1;
}
...
int RtpConnection::SendRtpOverTcp(MediaChannelId channel_id, const RtpPacket& pkt)
{
	...
	rtpPktPtr[0] = '$';
	rtpPktPtr[1] = (uint8_t)media_channel_info_[channel_id].rtp_channel;
