...
{
public:
    RtpConnection(std::weak_ptr<net::TcpConnection> rtsp_connection);
...
private:
    ...
    int  SendRtpOverTcp(MediaChannelId channel_id, const RtpPacket& pkt);
    int  SendRtpOverUdp(MediaChannelId channel_id, const RtpPacket& pkt);

    static std::random_device rd;
	std::weak_ptr<net::TcpConnection> rtsp_connection_;
    std::string rtsp_ip_;
    uint16_t rtsp_port_;
