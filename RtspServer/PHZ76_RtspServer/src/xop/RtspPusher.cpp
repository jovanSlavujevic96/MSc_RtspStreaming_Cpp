...
RtspPusher::RtspPusher(net::EventLoop *event_loop)
...
std::shared_ptr<RtspPusher> RtspPusher::Create(net::EventLoop* loop)
{
...
int RtspPusher::OpenUrl(std::string url, int msec)
{
	...
	static net::Timestamp timestamp;
	int timeout = msec;
	if (timeout <= 0) 
	{
		timeout = 10000;
	...
	{
		std::cerr << "rtsp url (" << url.c_str() << ") was illegal.\n";
		return -1;
	}
