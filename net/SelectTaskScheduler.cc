...
{	
	...
	{
		FD_ZERO(&fd_read_backup_);
		std::memcpy(&fd_read_backup_, &fd_read, sizeof(fd_set)); 
	}
	else
	{
		std::memcpy(&fd_read, &fd_read_backup_, sizeof(fd_set));
	}

	if(fd_write_reset)
	{
		FD_ZERO(&fd_write_backup_);
		std::memcpy(&fd_write_backup_, &fd_write, sizeof(fd_set));
	}
	else
	{
		std::memcpy(&fd_write, &fd_write_backup_, sizeof(fd_set));
	}

	if(fd_exp_reset)
	{
		FD_ZERO(&fd_exp_backup_);
		std::memcpy(&fd_exp_backup_, &fd_exp, sizeof(fd_set));
	}
	else
	{
		std::memcpy(&fd_exp, &fd_exp_backup_, sizeof(fd_set));
	}
