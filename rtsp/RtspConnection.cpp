...
RtspConnection::RtspConnection(std::shared_ptr<Rtsp> rtsp, net::TaskScheduler *task_scheduler, SOCKET sockfd) :
	TcpConnection(task_scheduler, sockfd) ,
	rtsp_(rtsp) ,
	task_scheduler_(task_scheduler) ,
	rtp_channel_(new net::Channel(sockfd)) ,
	rtsp_request_(new RtspRequest) ,
	rtsp_response_(new RtspResponse)
{
	this->SetReadCallback([this](std::shared_ptr<net::TcpConnection> conn, net::BufferReader& buffer) {
		return this->OnRead(buffer);
	});

	this->SetCloseCallback([this](std::shared_ptr<net::TcpConnection> conn) {
		this->OnClose();
	});
...
bool RtspConnection::OnRead(net::BufferReader& buffer)
{
	KeepAlive();
...
void RtspConnection::HandleRtcp(net::BufferReader& buffer)
{    
...
void RtspConnection::HandleCmdDescribe()
{
	...
	if(!rtsp || !media_session) {
		...
		std::string sdp = media_session->GetSdpMessage(net::SocketUtil::GetSocketIp(this->GetSocket()), rtsp->GetVersion());
		if(sdp == "") {
	...
	if(media_session->IsMulticast())  {
		...
		if(rtsp_request_->GetTransportMode() == RTP_OVER_TCP) {
			...
			if(rtp_conn_->SetupRtpOverUdp(channel_id, peer_rtp_port, peer_rtcp_port)) {
				...
				rtcp_channels_[channel_id].reset(new net::Channel(rtcp_fd));
