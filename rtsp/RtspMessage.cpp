...
bool RtspRequest::ParseRequest(net::BufferReader *buffer)
{
	if(buffer->Peek()[0] == '$') {
		method_ = RTCP;
		return true;
...
bool RtspRequest::ParseRequestLine(const char* begin, const char* end)
{
	...
	if(std::sscanf(message.c_str(), "%s %s %s", method, url, version) != 3) {
		return false; 
	}
...
bool RtspResponse::ParseResponse(net::BufferReader *buffer)
{
